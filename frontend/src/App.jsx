            ...
            <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                    ...
                    <button
                        ...
                        style={{ padding: '0.5rem 1rem', fontSize: '0.85rem', gap: '0.6rem' }}
                    >
                        <Search size={16} color="var(--text-violet)" />
                        <span style={{ display: 'none', mdDisplay: 'inline' }}>Search Vault</span>
                        <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', border: '1px solid var(--border-subtle)', padding: '1px 5px', borderRadius: '4px' }}>
                            Ctrl K
                        </span>
                    </button>
                    ...
                    {/* Vault AI Chat Assistant Trigger */}
                    <button
                    ...
                    {user ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            <span
                                ...
                                style={{ color: 'var(--text-violet)', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.9rem' }}
                            >
                                <User size={16} /> {user.username}
                            </span>
                            <button onClick={logout} className="btn btn-ghost" style={{ fontSize: '0.8rem', padding: '0.4rem 0.8rem' }}>
                                LOGOUT
                            </button>
                        </div>
                    ) : (
                        <button onClick={() => window.location.href = '/login'} className="btn btn-emerald" style={{ padding: '0.5rem 1.1rem', fontSize: '0.85rem' }}>
                            LOGIN
                        </button>
                    )}
                </div>
...
const HeroSection = ({ activeEra, setActiveEra, games = [] }) => (
    <header style={{
        ...
        position: 'relative',
        background: 'radial-gradient(ellipse 90% 60% at 50% -10%, rgba(168, 85, 247, 0.16), transparent 75%)',
        borderBottom: '1px solid var(--border-subtle)'
    }}>
        <div className="container animate-fade-in" style={{ textAlign: 'center', maxWidth: '1000px' }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.2rem' }}>
                <span className="badge badge-violet">
                    <Sparkles size={12} /> Framer Community Template Edition • Gemini & Groq
                </span>
            </div>
            ...
            <div style={{
                ...
                <div className="glass-panel" style={{ padding: '1rem', textAlign: 'center' }}>
                    <div style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--text-violet)' }}>25+ Masterpieces</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Indexed Eras (2000-2026)</div>
                </div>
                <div className="glass-panel" style={{ padding: '1rem', textAlign: 'center' }}>
                    <div style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--text-emerald)' }}>Internal .env Keys</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Gemini & Groq Auto-Loaded</div>
                </div>
                <div className="glass-panel" style={{ padding: '1rem', textAlign: 'center' }}>
                    <div style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--text-cyan)' }}>Rig Benchmark</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Instant 1080p/1440p/4K Meter</div>
                </div>
            </div>

            {/* Live AI Search Bar Widget */}
            <LiveAIFetcher />

            {/* Infinite Framer Community Marquee */}
            <FramerHeroMarquee games={games} />

            {/* Era Filter Pill Tabs */}
            <div style={{
                display: 'flex',
                gap: '0.6rem',
                justifyContent: 'center',
                ...
                {[
                    ...
                    { id: '2023-Present', label: 'New Era (2023–Present)' },
                    { id: '2020-2022', label: 'Next-Gen (2020–2022)' },
                    { id: '2015-2019', label: 'Modern Era (2015–2019)' },
                    { id: '2010-2014', label: 'HD Era (2010–2014)' },
                    { id: '2005-2009', label: 'Golden Age (2005–2009)' },
                    { id: '2000-2004', label: 'Classic Era (2000–2004)' }
                ].map(tab => (
                    <button
                        key={tab.id}
                        ...
                        style={{
                            ...
                            borderRadius: 'var(--radius-full)',
                            border: `1px solid ${activeEra === tab.id ? 'var(--accent-violet)' : 'var(--border-subtle)'}`,
                            background: activeEra === tab.id ? 'rgba(168, 85, 247, 0.15)' : 'rgba(255, 255, 255, 0.03)',
                            color: activeEra === tab.id ? 'var(--text-violet)' : 'var(--text-secondary)',
                            fontWeight: 700,
                            fontSize: '0.85rem',
                            cursor: 'pointer',
                            transition: 'all 0.2s'
    ...
    return (
        <section style={{ marginBottom: '4.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '1.8rem' }}>
                <div style={{ width: '4px', height: '28px', background: 'var(--accent-violet)', marginRight: '1rem', borderRadius: '2px' }}></div>
                <h2 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', margin: 0 }}>
                    {title}
                </h2>
                <span className="badge badge-dark" style={{ marginLeft: '1rem' }}>{games.length} Titles</span>
            </div>
...
const Home = ({ activeEra, games = [] }) => {
    ...
    return (
        <main className="container" style={{ paddingBottom: '4rem', paddingTop: '3rem' }}>
            <FramerFeatureCards />
        ...
        <Router>
            <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: 'var(--bg-canvas)' }}>
                ...
                <Routes>
                    <Route path="/" element={
                        <>
                            <HeroSection activeEra={activeEra} setActiveEra={setActiveEra} games={allGames} />
                            <Home activeEra={activeEra} games={allGames} />
                        </>
                    } />
